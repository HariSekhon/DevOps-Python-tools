...
check_output "google.com" ./find_active_server.py google.com yahoo.com --port 80
...
check_output "google.com" ./find_active_server.py google.com yahoo.com --http
...
check_output "google.com" ./find_active_server.py google.com yahoo.com --https

echo
echo "testing https returns no results when using wrong port 80"
echo

check_output "" ./find_active_server.py yahoo.com google.com --https --port 80
...
output="$(for x in {1..10}; do ./find_active_server.py google.com yahoo.com --random --https --regex google 2>&1; done)"
grep "google.com" <<< "$output" &&
! grep "yahoo.com" <<< "$output" ||
    die "FAILED: --random google + yahoo test, didn't return both results for 10 random HTTP runs"
