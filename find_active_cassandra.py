...
Tool to return the first active Cassandra node from an argument list of hosts
...
class FindActiveCassandra(FindActiveServer):

    def __init__(self):
        # Python 2.x
        super(FindActiveCassandra, self).__init__()
        # Python 3.x
        # super().__init__()
        # TCP only check - JMX port, could just as easily use CQL 9042
        self.default_port = 7199
        self.protocol = None

    def add_options(self):
        self.add_hostoption(name='Cassandra', default_port=self.default_port)
        #self.add_ssl_opt()
