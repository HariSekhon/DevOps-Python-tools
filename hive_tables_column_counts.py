...
Connect to HiveServer2 and count the number of columns for each table in each database,
...
class HiveTablesColumnCounts(HiveForEachTable):

    def __init__(self):
        # Python 2.x
        super(HiveTablesColumnCounts, self).__init__()
        # Python 3.x
        # super().__init__()
    ...
    def execute(self, conn, database, table, query):
        ...
        with conn.cursor() as column_cursor:
            ...
            for _ in column_cursor:
                #column = _[0]
                #column_type = _[1]
                column_count += 1
        print('{db}.{table}\t{column_count}'.format(db=database, table=table, column_count=column_count))


if __name__ == '__main__':
    HiveTablesColumnCounts().main()
