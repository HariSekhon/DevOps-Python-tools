...
Connect to an Impala daemon and count number of rows with only NULLs in all columns
for each tables in each database, or only those matching given db / table regexes
...
class ImpalaTablesNullRows(HiveTablesNullRows):

    def __init__(self):
        # Python 2.x
        super(ImpalaTablesNullRows, self).__init__()
        # Python 3.x
        # super().__init__()
